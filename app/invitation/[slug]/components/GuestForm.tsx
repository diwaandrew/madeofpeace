...
const GuestForm = () => {
  ...
  const handleSubmit = async (e: any) => {
    e.preventDefault();
    if (formData.name.trim() === '' || formData.attendance === '0' || formData.message.trim() === '') {
      setValidation(true)
      setTimeout(() => {
        setValidation(false)
      }, 1500)
  ...
  return (
    <Fragment>
      ...
      {validation && <CustomToast title={'Gagal Mengirim Pesan'} description={'Mohon lengkapi formulir sebelum mengirimkan pesan !'} />}
      {showSuccessModal && (
        <div className="fixed inset-0 z-10 overflow-y-auto">
          <div className="flex items-end justify-center min-h-14 pt-4 px-4 pb-20 text-center sm:block sm:p-0">
          ...
          <div className="flex items-end justify-center min-h-14 pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              <div className="bg-white px-4 pt-2 pb-4 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    <h3 className="text-lg font-medium text-gray-900" id="modal-title">{props.title}</h3>
                    <div className="mt-2">
